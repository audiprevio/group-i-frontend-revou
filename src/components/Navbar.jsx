...
const Navbar = () => {
  ...
  return (
    <div className="bg-white  py-4 pl-8 pr-4 flex flex-row items-center justify-between relative z-1000">
      <div className="flex flex-row justify-center items-center">
        ...
        <Button
          variant="light"
          onClick={handleOksigenMapClick}
          className="px-8 !text-sm font-medium text-oksigen-brand-blackX hover:text-oksigen-brand-blue !bg-none"
        >
          OksigenMap
        </Button>
        <Button
 ...
 hover:py-2 hover:px-4 px-4 py-2 hover:rounded-tremor-full transition-all duration-300 hover:text-oksigen-brand-blue"
          >
            {profile?.isPremium && (
              <Badge size="md" className="mr-4 font-normal">
                OksigenPlus
              </Badge>
            )}{" "}
            <b className="font-semibold">Halo, {profile?.organization_name}!</b>
          </Title>
        </div>
        {isCardVisible && (
